...
<div id="container">
	...
	<h2>Accediendo a Base de Datos!</h2>
                              ...
                              foreach($jugos as $row) { ?>
                                          <tr>
                                                      ...
                                                      <td><?php      $query = mysql_query("select fru.nombre from frutas fru join jugos_has_frutas jhf on fru.id = jhf.frutas_id where jhf.jugos_id = '".$row->id."' order by fru.nombre") or die(mysql_error());
                                                                        ...
                                                                        ?> </td>
                                                      <td><?php      $query = mysql_query("SELECT p.base FROM precios p, jugos j WHERE j.precios_id=p.id AND j.id = '".$row->id."'") or die(mysql_error());
                                                                        while($elemento= mysql_fetch_array($query)){
                                                                        echo $elemento['base'].'';
                                                                        }
                                                                        ?> </td>
                                                      <td><?php      $query = mysql_query("SELECT p.venta FROM precios p, jugos j WHERE j.precios_id=p.id AND j.id = '".$row->id."'") or die(mysql_error());
                                                                        while($elemento= mysql_fetch_array($query)){
                                                                        echo $elemento['venta'].'';
                                                                        }
                                                                        ?> </td>
